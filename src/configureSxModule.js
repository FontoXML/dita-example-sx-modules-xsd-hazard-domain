	...
	return function configureSxModule (sxModule) {
		...
		function getContextualOperationsForHazardType (hazardType) {
			var contextualOperations = [
				...
				{ name: ':hazardstatement-insert-messagepanel', hideIn: ['element-menu', 'breadcrumbs-menu'] },
				{ name: ':hazardstatement-append-hazardsymbol', hideIn: ['context-menu'] },
				{ name: ':hazardstatement-insert-hazardsymbol', hideIn: ['element-menu', 'breadcrumbs-menu'] }
			].concat(HAZARD_CONVERT_OPERATIONS
				.filter(function (element) {
					return element.name.indexOf(hazardType) === -1;
				})).concat([
				{ name: ':contextual-delete-hazardstatement' }
			]);
		...
		configureAsFrame(sxModule, 'self::messagepanel', t('message panel'), {
			contextualOperations: [
				...
				{ name: ':messagepanel-insert-consequence', hideIn: ['element-menu', 'breadcrumbs-menu'] },
				{ name: ':messagepanel-append-howtoavoid', hideIn: ['context-menu'] },
				{ name: ':messagepanel-insert-howtoavoid', hideIn: ['element-menu', 'breadcrumbs-menu'] },
